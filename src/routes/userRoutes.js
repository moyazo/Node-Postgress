...
router.get("/all-users", async (req, res) => {
  try {
    ...
    if (!allUsers) res.status(502).json("ERROR AT BRING DATA");
    else {
...
router.get("/user/:userEmail", async (req, res) => {
  try {
    const userByEmail = await getUserByEmail(req.params.userEmail);
    if (!userByEmail) res.status(502).json("ERROR AT BRING USER");
    else {
...
router.get("/user/:userId", async (req, res) => {
  try {
    ...
    if (!user) res.status(502).json("ERROR AT BRING DATA");
    else {
...
router.post("/createUser", async (req, res) => {
  try {
    ...
    if (!newUser) res.status(502).json("ERROR AT CREATE DATA");
    else {
...
router.put("/changeUser/:userId", async (req, res) => {
  try {
    ...
    if (!UserToChange) res.status(502).json("ERROR AT UPDATE DATA");
    else {
...
router.delete("/remove/:userId", async (req, res) => {
  try {
    ...
    if (!UserToDelete) res.status(502).json("ERROR AT DELETE DATA");
    else {
